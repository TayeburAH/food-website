...
class User(AbstractBaseUser, PermissionsMixin):
    email = models.EmailField(
        verbose_name='email address',  # to show its name in admin
        max_length=255,
        unique=True, validators=[validate_email_SMTP, ]  # list your own validator func here

    )
    date_joined = models.DateField(verbose_name='date joined',
                                   auto_now_add=True)  # when account gets created the date gets set
    last_joined = models.DateField(verbose_name='last joined',
                                   auto_now=True)  # when account gets created the date gets set
    ...
    def __str__(self):
        return self.username  # Django uses this when it needs to convert the object into string
...
class Customer(models.Model):
    ...
    zip = models.ForeignKey(Zip, on_delete=models.CASCADE, blank=True, null=True)
...
'''
AUTH_USER_MODEL = 'account.User'    #<app_name>.custom_model_name
